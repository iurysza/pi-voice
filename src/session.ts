...
export class VoiceLoop {
  ...
  private finishHandoff(output: string | undefined): void {
    ...
    if (callId) {
      const handoff: { callId: string; output?: string } = { callId };

      if (output) handoff.output = output;

      this.transport.completeHandoff(handoff);
    } else if (output) {
      this.transport.appendSpeech(output);
    }
  }
